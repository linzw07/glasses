<?php
class Gala_Bigshopsettings_Model_Observer {
	
	public function beforeGenerateBlocks(Varien_Event_Observer $observer) {
		if((Mage::getSingleton('core/design_package')->getPackageName() == 'default')&& (Mage::getDesign()->getTheme('frontend') == 'galabigshop')){	
			# Disable default magento navigation
			if (Mage::helper('bigshopsettings')->getGeneral_DisableDefaultNav()&&(Mage::getConfig()->getModuleConfig('EM_Megamenupro')->is('active', 'true'))) {
				$blocks = $observer->getLayout()->getXpath('//block[@name="galabigshop.catalog.topnav"]');
				if (!empty($blocks))
					$blocks[0]->addAttribute('ignore', true);
                $blocks = $observer->getLayout()->getXpath('//block[@name="galabigshop.catalog.topnav.left"]');
				if (!empty($blocks))
					$blocks[0]->addAttribute('ignore', true);
			}
		
			# Disable EM variation module on frontend
			if (Mage::helper('bigshopsettings')->getGeneral_DisableFrontendVariation()) {
			...
			if (Mage::helper('bigshopsettings')->getGeneral_DisableFooterLinks()) {
	...
	public function beforeCatalogProductCollectionLoad(Varien_Event_Observer $observer) {
		if((Mage::getSingleton('core/design_package')->getPackageName() == 'default')&& (Mage::getDesign()->getTheme('frontend') == 'galabigshop')){		
			$alt_img = Mage::helper('bigshopsettings')->getProductsGrid_AltImg();
			if ($alt_img == 'image')
